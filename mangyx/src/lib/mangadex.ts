...
export function getCoverUrl(manga: Manga, size: '256' | '512' | '' = '256'): string {
  ...
  return `${COVER_BASE}/${manga.id}/${rel.attributes.fileName}${suffix}`
}
...
export async function searchManga(
  ...
  const data = await get<MangaDexListResponse<Manga>>('/manga', params)
  return { data: data.data, total: data.total }
}

export async function getMangaById(id: string): Promise<Manga> {
  const data = await get<{ data: Manga }>(`/manga/${id}`, {
    'includes[]': ['cover_art'],
  })
  return data.data
}
...
export async function getMangaFeed(
  ...
  const data = await get<MangaDexListResponse<Chapter>>(`/manga/${mangaId}/feed`, {
    limit: String(limit),
    offset: String(offset),
  ...
  return { chapters: data.data.filter((c) => c.attributes.chapter !== null), total: data.total }
}
...
export async function getMangaTags(): Promise<{ id: string; name: string }[]> {
  const data = await get<{ data: { id: string; attributes: { name: { en: string }; group: string } }[] }>('/manga/tag')
  return data.data
